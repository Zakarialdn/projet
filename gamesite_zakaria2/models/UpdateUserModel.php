...
class UpdateUserModel
{
    ...
    public function updateProfile($firstname, $lastname, $email, $password)
    {
        ...
        $stmt = $this->pdo->query("UPDATE users SET Firstname = ?, Lastname = ?, Email = ?, Password = ? WHERE id = ?");

        // Exécuter la requête avec les nouvelles valeurs
        $result = $this->stmt->execute([$firstname, $lastname, $email, $password, $userId]);
